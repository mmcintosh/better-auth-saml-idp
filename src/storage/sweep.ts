 ...
 *  - `samlIdpAuditEvent` rows past `auditLog.retentionDays` (D-038);
 ...
 */
import { AUDIT_MODEL } from "../events";
...
export async function sweepExpired(adapter: Adapter, onError: (what: string, e: unknown) => void, now = Date.now(), opts: { participants?: boolean; auditLog?: boolean } = {}) {
  ...
  if (opts.auditLog) await adapter.deleteMany({ model: AUDIT_MODEL, where }).catch((e) => onError("audit log", e));
}
