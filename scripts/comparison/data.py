...
FEATURES = [
 ...
 ("Signing and encryption", "Verifies and can require signed AuthnRequests", row(("p","HTTP-Redirect only, per SP"),("y",""),("y",""),("y",""),("p","instance-wide switch"),("y",""),("y",""),("n","checks the key embedded in the request"),("y",""),("y",""),("u",""),("p","can verify; requiring not documented")), "test/integration/security.test.ts, review-findings.test.ts #2"),
 ...
 ("Signing and encryption", "Signing key rotation", row(("p","publishes extra certificates; switch by hand"),("p","manual"),("y","two-key rollover"),("y","active and passive keys"),("y","automatic"),("p","manual"),("p","one active at a time"),("p","single key"),("y","expiry emails"),("y",""),("y","two certificates"),("p","tenant-wide key")), ""),
 ...
 ("Replay and protocol fidelity", "ForceAuthn, IsPassive, RequestedAuthnContext", row(("y","exact match on one configured class"),("y",""),("p",""),("y","step-up via LoA"),("n",""),("p","ForceAuthn only"),("p","ForceAuthn only"),("n",""),("y","RequestedAuthnContext exact only"),("p","ForceAuthn setting"),("u",""),("p","no IsPassive")), "test/integration/review-findings.test.ts #12"),
