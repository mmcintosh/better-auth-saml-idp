...
| Weak crypto | RSA-SHA256 / SHA-256 by default. SHA-1 needs `allowInsecureSha1: true` and logs a warning. RSA keys under 2048 bits and mismatched key/cert pairs are rejected at startup; an expired or soon-expiring certificate is logged as a warning (failing would take every route down the moment it expires). | `options.test.ts` |
