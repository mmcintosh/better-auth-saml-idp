...
{ id: "zoom", entityId: "…", acsUrls: ["…"],
  organization: { slug: "acme", roles: ["admin", "member"] },    // members only (with one of these roles)
  attributes: {
    email: "email",
    groups: { organization: "slugs" },                           // every org the user belongs to
    roles: { organization: "roles" },                            // roles in "acme" (else "slug:role" for all)
  } }
```
