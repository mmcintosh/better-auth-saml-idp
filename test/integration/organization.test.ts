...
async function host(sp: Record<string, unknown>, o: { withPlugin?: boolean; authorize?: (c: any) => boolean } = {}) {
  const { auth } = await createHost({
    plugins: o.withPlugin === false ? [] : [organization()],
    saml: { serviceProviders: [{ id: "sp", entityId: SP_ENTITY_ID, acsUrls: [SP_ACS], ...(o.authorize ? { authorize: o.authorize } : {}), ...sp } as any] },
  });
...
describe.skipIf(isWorkerd)("organization plugin", () => {
  ...
  it("organization attributes: slugs and names everywhere; roles scoped to the SP's organization", async () => {
    const { browser, org } = await host({
      organization: { slug: "acme" },
      attributes: { email: "email", groups: { organization: "slugs" }, orgNames: { organization: "names" }, roles: { organization: "roles" } },
    });
    ...
    expect(values("groups").sort()).toEqual(["acme", "globex"]);
    expect(values("orgNames").sort()).toEqual(["ACME", "GLOBEX"]);
  ...
  it("an SP that requires an organization fails closed without the organization plugin", async () => {
    const { browser } = await host({ organization: { slug: "acme" } }, { withPlugin: false });
    expect(await code(await sso(browser))).toBe("ACCESS_DENIED");
  });
